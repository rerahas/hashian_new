...
if ($$in{match}) {
	...
	if ($$in{$match1} ne $$in{$match2}) {
		error("メールアドレスとメールアドレス再入力内容が異なります");
	}
}
...
sub send_mail {
	...
	if ($cf{reload}) {
		if ($ENV{PERLXS} eq "PerlIS") {
			print "https/1.0 302 Temporary Redirection\r\n";
...
#-----------------------------------------------------------
#  mimeエンコード
#  [quote] https://www.din.or.jp/~ohzaki/perl.htm#JP_Base64
#-----------------------------------------------------------
